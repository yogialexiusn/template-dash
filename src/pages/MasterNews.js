...
import { MASTER_NEWS_LIST, MASTER_NEWS_ADD } from "../config/Constants";
...
function MasterNews() {
  ...
  const handleFilterButton = useCallback(async () => {
    ...
    try {
      let response = await axiosInstance().post(MASTER_NEWS_LIST, requestBody);
  ...
  const handleEdit = async (modifiedInput) => {
    for (const obj of modifiedInput) {
      ...
      try {
        let response = await axiosInstance().post(MASTER_NEWS_ADD, requestBody);
        if (response.status === 200) {
  ...
  return (
    <React.Fragment>
      ...
      <Content page="component">
        <BlockHead>
          <BlockHeadContent>
            <BlockTitle>Master News</BlockTitle>
            <BlockDes>
              <p>Master News is used to created new Job for applicant.</p>
            </BlockDes>
          </BlockHeadContent>
        </BlockHead>
