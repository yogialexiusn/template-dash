...
import { MASTER_MENU_LIST, MASTER_MENU_ADD } from "../config/Constants";
...
function MasterMenu() {
  ...
  const handleEdit = async (modifiedInput) => {
    for (const obj of modifiedInput) {
      ...
      try {
        let response = await axiosInstance().post(MASTER_MENU_ADD, requestBody);
        if (response.status === 200) {
  ...
  return (
    <React.Fragment>
      ...
      <Content page="component">
        <BlockHead>
          <BlockHeadContent>
            <BlockTitle>Master Menu</BlockTitle>
            <BlockDes>
              <p>Master Menu is used to created new Job for applicant.</p>
            </BlockDes>
          </BlockHeadContent>
        </BlockHead>
