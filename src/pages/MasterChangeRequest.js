...
import { MASTER_CHANGEREQUEST_LIST, MASTER_CHANGEREQUEST_ADD } from "../config/Constants";

function MasterChangeRequest() {
  ...
  const handleFilterButton = useCallback(async () => {
    ...
    try {
      let response = await axiosInstance().post(MASTER_CHANGEREQUEST_LIST, requestBody);
  ...
  const handleEdit = async (modifiedInput) => {
    for (const obj of modifiedInput) {
      ...
      try {
        let response = await axiosInstance().post(MASTER_CHANGEREQUEST_ADD, requestBody);
        if (response.status === 200) {
  ...
  return (
    <React.Fragment>
      ...
      <Content page="component">
        <BlockHead>
          <BlockHeadContent>
            <BlockTitle>Master Change Request</BlockTitle>
            <BlockDes>
              <p>Master Change Request is used to created new Job for applicant.</p>
            </BlockDes>
          </BlockHeadContent>
        </BlockHead>
...
export default MasterChangeRequest;
