...
import { MASTER_ROLE_LIST, MASTER_ROLE_ADD } from "../config/Constants";

function TrcChangeRequest() {
  ...
  const handleEdit = async (modifiedInput) => {
    for (const obj of modifiedInput) {
      ...
      try {
        let response = await axiosInstance().post(MASTER_ROLE_ADD, requestBody);
        if (response.status === 200) {
  ...
  return (
    <React.Fragment>
      ...
      <Content page="component">
        <BlockHead>
          <BlockHeadContent>
            <BlockTitle>Change Request </BlockTitle>
            <BlockDes>
              <p>Change Request is used to created new Change Request</p>
            </BlockDes>
          </BlockHeadContent>
        </BlockHead>
...
export default TrcChangeRequest;
