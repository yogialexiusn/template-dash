...
import { MASTER_PROGRAM_LIST, MASTER_PROGRAM_ADD } from "../config/Constants";
...
function MasterProgram() {
  ...
  const handleFilterButton = useCallback(async () => {
    ...
    try {
      let response = await axiosInstance().post(MASTER_PROGRAM_LIST, requestBody);
  ...
  const handleEdit = async (modifiedInput) => {
    for (const obj of modifiedInput) {
      ...
      try {
        let response = await axiosInstance().post(MASTER_PROGRAM_ADD, requestBody);
        if (response.status === 200) {
  ...
  return (
    <React.Fragment>
      ...
      <Content page="component">
        <BlockHead>
          <BlockHeadContent>
            <BlockTitle>Master Program</BlockTitle>
            <BlockDes>
              <p>Master Program is used to created new Job for applicant.</p>
            </BlockDes>
          </BlockHeadContent>
        </BlockHead>
